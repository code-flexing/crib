...
export default function AdminReviewPage() {
  ...
  const decide = async (status: "APPROVED" | "REJECTED") => {
    ...
    try {
      const response = unwrapData<Partial<ReviewSubmission>>(await adminFetch<unknown>("/api/v1/admin/review", { method: "POST", body: JSON.stringify({ submissionId: review?.id ?? id, status, ...(status === "REJECTED" ? { reason: trimmedReason } : {}) }) }));
